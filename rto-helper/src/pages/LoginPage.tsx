...
async function postAgentLogin(body: { email: string; password: string }) {
  const { data } = await axios.post<AgentLoginResponse>(
...
export function LoginPage() {
  ...
  const setAgentSession = useAuthStore((s) => s.setAgentSession)
  ...
  const loginMutation = useMutation({
    mutationFn: postAgentLogin,
    onSuccess: (data) => {
      const ok = setAgentSession(data.accessToken, data.refreshToken)
  ...
  return (
    <div className="mx-auto w-full max-w-md space-y-8">
      <div className="text-center">
        <h1 className="bg-gradient-to-r from-accent-blue to-accent-green bg-clip-text text-3xl font-bold tracking-tight text-transparent">
          RTO Helper
        </h1>
