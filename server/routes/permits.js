...
router.get('/', async (req, res) => {
  ...
  const [rows, stats] = await Promise.all([
    ...
    statsForModel({
      tenantId,
      prismaModel: prisma.permitRecord,
...
router.post('/', async (req, res) => {
  ...
  const permitType = typeof body.permitType === 'string' ? body.permitType.trim() : ''
  const issuedState = typeof body.issuedState === 'string' ? body.issuedState.trim() : ''
  if (!vehicleId || !permitType || !issuedState) {
    return res.status(400).json({ message: 'vehicleId, permitType, and issuedState are required' })
  }
  ...
  const row = await prisma.permitRecord.create({
    data: {
      tenantId,
      vehicleId,
      permitType,
...
router.put('/:id', async (req, res) => {
  const tenantId = req.tenant.id
  const { id } = req.params
  const existing = await prisma.permitRecord.findFirst({
    where: { id, tenantId },
  })
  ...
  if (typeof body.permitType === 'string') data.permitType = body.permitType.trim()
  if (typeof body.issuedState === 'string') data.issuedState = body.issuedState.trim()
  if (body.expiryDate) {
  ...
  const row = await prisma.permitRecord.update({
...
router.delete('/:id', async (req, res) => {
  ...
  await prisma.permitRecord.delete({ where: { id: existing.id } })
  res.status(204).send()
})
